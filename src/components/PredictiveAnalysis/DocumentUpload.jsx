...
import { analyzeImageAsync, analyzeFileAsync } from "../../features/Admin/PredictiveAnalysis/predictiveAnalysisSlice";

const DocumentUpload = ({ onAnalyze, isLoading }) => {
  const [uploadedFiles, setUploadedFiles] = useState([]);
  const fileInputRef = useRef(null);
  const dispatch = useDispatch();
  const { loading, error } = useSelector((state) => state.predictiveAnalysis);
    ...
    if (uploadedFiles.length > 0) {
      ...
      try {
        if (imageExtensions.includes(fileExtension)) {
          ...
          dispatch(analyzeFileAsync({ file, docType: "report_card" }));
        }
        onAnalyze(file);
      } catch (err) {
  ...
  return (
    <div className="w-full">
      ...
      <p className="text-xs sm:text-sm text-gray-600 mb-4 sm:mb-6">
        Home / Predictive Analysis Dashboard
      </p>
      ...
      <div className="bg-white rounded-lg p-4 sm:p-8 border border-gray-200 shadow-sm mb-4 sm:mb-6">
        <p className="text-gray-700 font-semibold mb-4 sm:mb-6 text-sm sm:text-base">
          Upload student data for AI-powered performance insights and risk assessment
        </p>
        ...
        >
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-gray-300 text-xs sm:text-sm mb-3 sm:mb-4">
              Report card images, CSV, JSON, PDF, Excel or text files
            </p>
          ...
          <div className="flex items-start gap-3 sm:gap-4">
            ...
            <div>
              ...
              <p className="text-gray-700 text-xs sm:text-sm">
                OCR technology identifies tabular data from scanned report cards with
        ...
        <div className="bg-red-100 rounded-lg p-4 sm:p-6 border border-red-200">
          <div className="flex items-start gap-3 sm:gap-4">
            ...
            <div>
              ...
              <p className="text-gray-700 text-xs sm:text-sm">
                Our models compare historical performance against 1.2M student data
                points for accuracy.
              </p>
            </div>
          </div>
        ...
        <div className="bg-green-100 rounded-lg p-4 sm:p-6 border border-green-200">
          <div className="flex items-start gap-3 sm:gap-4">
            ...
            <div>
              ...
              <p className="text-gray-700 text-xs sm:text-sm">
                Generates specific interventions for students flagged at high-risk of
                academic attrition.
              </p>
            </div>
          </div>
