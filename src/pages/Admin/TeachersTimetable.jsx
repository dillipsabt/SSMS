...
const itemTime = (item) => `${formatTime(item.startTime ?? item.timeSlot?.startTime)} - ${formatTime(item.endTime ?? item.timeSlot?.endTime)}`;
...
export default function TeachersTimetable() {
  ...
  const toggleSelected = (id) => setSelectedRows((selected) => selected.includes(id) ? selected.filter((item) => item !== id) : [...selected, id]);
  ...
  return <div>
    ...
    <div className="card">
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex justify-end mt-4"><button onClick={() => selectedRows.length ? setPublishOpen(true) : toast.error("Select at least one schedule")} className="btn-primary">Publish</button></div>
      </div>
    </div>
