import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { Search, MoreVertical } from "lucide-react";
import { useDispatch, useSelector } from "react-redux";
import {
  getStaffAsync,
  resetStaffState,
  deleteStaffAsync,
  clearSuccess,
  clearError,
} from "../../features/Admin/Staff/staffSlice";

import Pagination from "../../components/common/Pagination";
import DeleteConfirmModal from "../../components/common/DeleteConfirmModal";
import useToastMessage from "../../utils/useToastMessage";
 
const StaffList = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const canManageStaff = localStorage.getItem("role") !== "staff-portal";
 
  const [currentPage, setCurrentPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [deleteModal, setDeleteModal] = useState(false);
  const [selectedId, setSelectedId] = useState(null);
  const [deleteType, setDeleteType] = useState("");
 
  // Track menus using unique record IDs instead of generic indices
  const [openMenuId, setOpenMenuId] = useState(null);
  const [menuPos, setMenuPos] = useState({ top: 0, left: 0 });
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("All");
 
  // 1. ADDED: State for managing selected department filter
  const [departmentFilter, setDepartmentFilter] = useState("All");
 
  const { staff, message, error, success } = useSelector((state) => state.staff);

  useEffect(() => {
    dispatch(clearSuccess());
    dispatch(clearError());
  }, [dispatch]);

  useEffect(() => {
    dispatch(getStaffAsync());
    // Cleanup state on unmount
    return () => {
      dispatch(resetStaffState());
    };
  }, [dispatch]);

  // 2. UPDATED: Reset pagination when department changes too
  useEffect(() => {
    setCurrentPage(1);
  }, [search, statusFilter, departmentFilter]);

  useToastMessage({
    success,
    error,
    successMessage: typeof message === "string" ? message : message?.message || "Operation successful",
    clearSuccess,
    clearError,
  });
 
  const formatted = Array.isArray(staff)
    ? staff.map((s) => ({
        id: s.id,
        staffId: s.staffId || "-",
        name: s.fullName || "-",
        department: s.departmentName || "-",
        designation: s.designation || "-",
        phone: s.phoneNumber || "-",
        email: s.email || "-",
        date: s.joinDate || "-",
        status: s.active ? "Active" : "Inactive",
      }))
    : [];
 
  // 3. ADDED: Extract unique departments dynamically from formatted data
  const uniqueDepartments = [
    "All",
    ...new Set(
      formatted.map((s) => s.department).filter((dept) => dept && dept !== "-"),
    ),
  ];
 
  const handleDeleteClick = (id, type = "staff") => {
    setSelectedId(id);
    setDeleteType(type);
    setDeleteModal(true);
  };
 
  const confirmDelete = () => {
    if (selectedId) {
      dispatch(deleteStaffAsync(selectedId));
    }
    setDeleteModal(false);
    setSelectedId(null);
  };
 
  // 4. UPDATED: Filter logic to handle departmentFilter matching
  const filteredStaff = formatted.filter((s) => {
    const matchesSearch = s.name.toLowerCase().includes(search.toLowerCase());
 
    const matchesStatus =
      statusFilter === "All"
        ? true
        : s.status?.toLowerCase() === statusFilter.toLowerCase();
 
    const matchesDepartment =
      departmentFilter === "All"
        ? true
        : s.department?.toLowerCase() === departmentFilter.toLowerCase();
 
    return matchesSearch && matchesStatus && matchesDepartment;
  });
 
  const indexOfLast = currentPage * rowsPerPage;
  const indexOfFirst = indexOfLast - rowsPerPage;
  const currentStaff = filteredStaff.slice(indexOfFirst, indexOfLast);
  const totalPages = Math.ceil(filteredStaff.length / rowsPerPage);
 
  const toggleMenu = (e, id) => {
    if (openMenuId === id) {
      setOpenMenuId(null);
    } else {
      const rect = e.currentTarget.getBoundingClientRect();
      setMenuPos({
        top: rect.bottom + window.scrollY + 5,
        left: rect.left + window.scrollX - 80,
      });
      setOpenMenuId(id);
    }
  };
 
  return (
    <div>
      <h2 className="text-[18px] font-semibold text-[#333333]">Staff List</h2>
      <p className="text-[11px] sm:text-[12px] text-gray-500 mb-4">
        Staff / Staff List
      </p>
 
      <div className="card p-3 sm:p-4">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3">
          <h2 className="text-sm font-medium text-gray-700">Staff List</h2>
 
          <div className="flex flex-col sm:flex-row gap-2 sm:items-center w-full sm:w-auto">
            <div className="relative w-full sm:w-[200px]">
              <input
                type="text"
                placeholder="Search"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="border border-gray-300 text-[12px] rounded px-3 py-[6px] pr-7 w-full focus:outline-none"
              />
              <Search
                size={12}
                className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400"
              />
            </div>
 
            {/* 5. ADDED: Department selection element dropdown */}
            <select
              value={departmentFilter}
              onChange={(e) => setDepartmentFilter(e.target.value)}
              className="border border-gray-300 text-[12px] rounded px-2 py-[6px] w-full sm:w-[140px] focus:outline-none bg-white"
            >
              {uniqueDepartments.map((dept) => (
                <option key={dept} value={dept}>
                  {dept}
                </option>
              ))}
            </select>
 
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="border border-gray-300 text-[12px] rounded px-2 py-[6px] w-full sm:w-[120px] focus:outline-none bg-white"
            >
              <option value="All">All Statuses</option>
              <option value="Active">Active</option>
              <option value="Inactive">Inactive</option>
            </select>
          </div>
        </div>
 
        {/* DESKTOP VIEW */}
        <div className="hidden lg:block border border-gray-300 rounded overflow-hidden">
          <table className="w-full text-[12px]">
            <thead className="thead-row bg-gray-50">
              <tr>
                <th className="px-4 py-2 text-left">S No.</th>
                <th className="px-4 py-2 text-left">ID No.</th>
                <th className="px-4 py-2 text-left">Name</th>
                <th className="px-4 py-2 text-left">Department</th>
                <th className="px-4 py-2 text-left">Designation</th>
                <th className="px-4 py-2 text-left">Phone No.</th>
                <th className="px-4 py-2 text-left">Email</th>
                <th className="px-4 py-2 text-left">Join Date</th>
                <th className="px-4 py-2 text-left">Status</th>
                <th className="px-4 py-2 text-left">Action</th>
              </tr>
            </thead>
 
            <tbody>
              {currentStaff.map((item, index) => (
                <tr
                  key={item.id || index}
                  className="border-t border-gray-200 hover:bg-gray-50"
                >
                  <td className="px-4 py-2">{indexOfFirst + index + 1}</td>
                  <td className="px-4 py-2">{item.staffId}</td>
                  <td className="px-4 py-2 font-medium">{item.name}</td>
                  <td className="px-4 py-2">{item.department}</td>
                  <td className="px-4 py-2">{item.designation}</td>
                  <td className="px-4 py-2">{item.phone}</td>
                  <td className="px-4 py-2">{item.email}</td>
                  <td className="px-4 py-2">{item.date}</td>
 
                  <td className="px-4 py-2">
                    <span
                      className={`px-2 py-1 text-xs rounded ${
                        item.status === "Active"
                          ? "bg-green-100 text-green-600"
                          : "bg-red-100 text-red-600"
                      }`}
                    >
                      {item.status}
                    </span>
                  </td>
 
                  <td className="px-4 py-2 relative">
                    <MoreVertical
                      size={16}
                      className="cursor-pointer"
                      onClick={(e) => toggleMenu(e, item.id)}
                    />
 
                    {openMenuId === item.id && (
                      <div
                        style={{
                          position: "fixed",
                          top: menuPos.top,
                          left: menuPos.left,
                          zIndex: 9999,
                        }}
                        className="w-28 bg-white border rounded shadow"
                      >
                        <button
                          onClick={() => {
                            navigate(`/staff-details/${item.id}`);
                            setOpenMenuId(null);
                          }}
                          className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-100"
                        >
                          View
                        </button>
 
                        {canManageStaff && (
                          <>
                            <button
                              onClick={() => {
                                navigate(`/add-staff/${item.id}`);
                                setOpenMenuId(null);
                              }}
                              className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-100"
                            >
                              Edit
                            </button>

                            <button
                              onClick={() => {
                                handleDeleteClick(item.id, "staff");
                                setOpenMenuId(null);
                              }}
                              className="block w-full text-left px-3 py-2 text-sm text-red-500 hover:bg-gray-100"
                            >
                              Delete
                            </button>
                          </>
                        )}
                      </div>
                    )}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
 
        {/* MOBILE VIEW */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:hidden">
          {currentStaff.map((item, index) => (
            <div
              key={item.id || index}
              className="border rounded p-3 bg-white relative"
            >
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-medium text-sm">{item.name}</p>
                  <p className="text-xs text-gray-500">{item.designation}</p>
                </div>
 
                <div className="relative">
                  <MoreVertical
                    size={16}
                    className="cursor-pointer"
                    onClick={(e) => toggleMenu(e, item.id)}
                  />
 
                  {openMenuId === item.id && (
                    <div
                      style={{
                        position: "fixed",
                        top: menuPos.top,
                        left: menuPos.left,
                        zIndex: 9999,
                      }}
                      className="w-28 bg-white border rounded shadow text-xs"
                    >
                      <button
                        onClick={() => {
                          navigate(`/staff-details/${item.id}`);
                          setOpenMenuId(null);
                        }}
                        className="block w-full text-left px-3 py-2 hover:bg-gray-100"
                      >
                        View
                      </button>
 
                      {canManageStaff && (
                        <>
                          <button
                            onClick={() => {
                              navigate(`/add-staff/${item.id}`);
                              setOpenMenuId(null);
                            }}
                            className="block w-full text-left px-3 py-2 hover:bg-gray-100"
                          >
                            Edit
                          </button>

                          <button
                            onClick={() => {
                              handleDeleteClick(item.id, "staff");
                              setOpenMenuId(null);
                            }}
                            className="block w-full text-left px-3 py-2 text-red-500 hover:bg-gray-100"
                          >
                            Delete
                          </button>
                        </>
                      )}
                    </div>
                  )}
                </div>
              </div>
 
              <div className="mt-2 text-xs space-y-1 text-gray-600">
                <p>
                  <b>ID:</b> {item.staffId}
                </p>
                <p>
                  <b>Department:</b> {item.department}
                </p>
                <p>
                  <b>Phone:</b> {item.phone}
                </p>
                <p>
                  <b>Email:</b> {item.email}
                </p>
                <p>
                  <b>Date:</b> {item.date}
                </p>
              </div>
 
              <div className="mt-2">
                <span
                  className={`px-2 py-1 text-xs rounded ${
                    item.status === "Active"
                      ? "bg-green-100 text-green-600"
                      : "bg-red-100 text-red-600"
                  }`}
                >
                  {item.status}
                </span>
              </div>
            </div>
          ))}
        </div>
 
        {/* PAGINATION */}
        <Pagination
          currentPage={currentPage}
          totalPages={totalPages}
          rowsPerPage={rowsPerPage}
          setCurrentPage={setCurrentPage}
          setRowsPerPage={setRowsPerPage}
        />
      </div>
 
      <DeleteConfirmModal
        isOpen={deleteModal}
        title="Delete Staff"
        message={`Are you sure you want to delete this ${deleteType}?`}
        onClose={() => {
          setDeleteModal(false);
          setSelectedId(null);
        }}
        onConfirm={confirmDelete}
      />
    </div>
  );
};
 
export default StaffList;
